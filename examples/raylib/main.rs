...
/// Reimui drawing
fn draw_ui<'a>(rl: &RaylibHandle, font_info: &'a RaylibFontInfo) -> UIContext<'a> {
    ...
    ctx.draw_text_layout(&mut layout, "reimui + raylib".into());
    ctx.draw_text_layout(&mut layout, "i love user interfaces".into());
    let clicked = ctx.draw_button(&mut layout, Vec2 { x: 18, y: 12 }, "Click this".into());
    if clicked {
        ctx.draw_text_layout(&mut layout, "hell yeah".into());
    }
    ctx
}

/// Raylib window & loop, just draws our UI
fn main() {
    let (mut rl, thread) = raylib::init().size(640, 400).title("r e i m u i").build();

    let font_info = RaylibFontInfo { font_size: 22 };

    while !rl.window_should_close() {
        // draw ui
        let mut ctx = draw_ui(&rl, &font_info);

        // transfer all draw commands into raylib as our renderer
