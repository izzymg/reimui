...
pub enum DrawCommand {
    DrawText { content: String, x: u32, y: u32 },
}
...
impl<'s, 'f> UIContext< 'f> {
    pub fn new(font_info: &'f dyn FontInformation) -> Self {
        UIContext {
...
mod test {
    ...
    fn layout() {
        ...
        let mut ctx = super::UIContext::new(&font_info);
        ...
        for i in 0..3 {
            ...
            assert!(label.len() as u32 == SECTION_TEXT_LEN, "broken test assertion");
        ...
        assert_eq!(layout.top_left.x, 3 * (SECTION_TEXT_LEN * MOCK_TEXT_WIDTH + layout.spacing));
